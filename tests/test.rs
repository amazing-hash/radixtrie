...
fn test_radix_trie_7() {
    ...
    for _ in 0..10 {
        ...
        for _ in 0..1000 {
            ...
            assert_eq!(
                radix_trie.lookup(keys[idx as usize].as_bytes()),
                Some(&true)
            );
        }
        for key in &keys {
...
fn test_radix_trie_12() {
    ...
    assert_eq!(
        keys,
        vec!["aaa".to_string(), "aaab".to_string(), "aaabc".to_string()]
    );
}
#[test]
...
fn test_radix_trie_14() {
    ...
    assert_eq!(
        keys,
        vec!["aaab".to_string(), "aaac".to_string(), "aaad".to_string()]
    );
}
#[test]
