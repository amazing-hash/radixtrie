    ...
    pub fn iter<'a>(&'a self, prefix: &'a [u8]) -> RadixTrieIter<'a, T> {
        ...
        if prefix.is_empty() {
            ...
            while let Some(next_node) = node {
                if inner_prefix.len() <= next_node.key.len()
                    && next_node.key.starts_with(inner_prefix)
                {
                    queue.push_back((
                        prefix[..prefix.len() - inner_prefix.len()].to_vec(),
                        next_node,
                    ));
                    break;
...
impl<T> NodeTrie<T> {
            ...
            if idx < self.key.len() {
                ...
                unsafe {
                    self.children
                        .as_mut()
                        .unwrap_unchecked()
                        .insert(self.key[idx], node);
                }
                self.key.drain(idx..);
            }
            if let Some(next) =
                unsafe { self.children.as_mut().unwrap_unchecked().get_mut(&key[idx]) }
            {
    ...
    fn lookup(&self, key: &[u8]) -> Option<&T> {
        if !key.is_empty() {
            ...
            if idx == self.key.len() {
                if let Some(next) =
                    unsafe { self.children.as_ref().unwrap_unchecked().get(&key[idx]) }
                {
